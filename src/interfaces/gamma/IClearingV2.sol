...
interface IClearingV2 {
    ...
    function applyRatio(address pos, address token, uint256 total0, uint256 total1)
        external
        view
        returns (uint256 ratioStart, uint256 ratioEnd);
    function checkPriceChange(address pos, uint32 _twapInterval, uint256 _priceThreshold)
        external
        view
